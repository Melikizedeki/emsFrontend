...
const Report = () => {
  ...
  const fetchPerformance = async () => {
    ...
    try {
      ...
      const filtered = res.data.filter(
        (emp) => emp.role !== "admin" && emp.role !== "field"
      );
  ...
  const fetchPayroll = async () => {
    ...
    try {
      ...
      const totalSalary = filtered.reduce((sum, d) => sum + Number(d.salary || 0), 0);
      const totalNetSalary = filtered.reduce((sum, d) => sum + Number(d.net_salary || 0), 0);
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <style>{`
        @media print {
          ...
          .no-print, .header-ui, .filter-section, .pagination { display: none !important; }
          #print-area { display: block !important; width: 100% !important; margin: 0; padding: 0; }
          table, th, td { border: 1px solid black !important; border-collapse: collapse !important; }
          .print-logo { width: 120px; margin: 0 auto 10px auto; display: block; }
        ...
        <div id="print-area" className="bg-white p-6 rounded shadow">
          ...
          <div className="print-only mb-4 text-center">
            ...
            <h2 className="font-semibold mt-1">
              PERIOD: {period === "full" ? `Full Year ${year}`
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6 filter-section">
            ...
            <input type="text" value={holidays} placeholder="YYYY-MM-DD, comma separated" onChange={(e) => setHolidays(e.target.value)} className="border p-2 rounded" />
          </div>

          <button onClick={fetchPerformance} className="bg-blue-600 text-white px-4 py-2 rounded mb-6 no-print">Generate Report</button>
          ...
          <div className="overflow-x-auto">
            <table className="w-full border">
              ...
              <tbody>
                {currentPerformance.length > 0 ? currentPerformance.map((emp, i) => (
            ...
            {performanceData.length > itemsPerPage && (
              <div className="flex justify-between items-center mt-4 pagination no-print">
                ...
                <button disabled={currentPage === totalPagesPerformance} onClick={() => setCurrentPage(p => p + 1)} className="px-3 py-1 bg-gray-200 rounded disabled:opacity-50">Next</button>
              </div>
            )}
          </div>
        </div>
      ...
      {activeTab === "payroll" && (
        <div className="bg-white p-6 rounded shadow">
          ...
          <button onClick={fetchPayroll} className="bg-green-600 text-white px-4 py-2 rounded mb-6">Generate Report</button>
          ...
          <div className="overflow-x-auto">
            <table className="w-full border">
              ...
              <tbody>
                {currentPayroll.length > 0 ? currentPayroll.map((p, i) => (
                  <tr key={i} className="border-t">
                    ...
                    <td className="py-2 px-3 text-center text-green-600">{Number(p.net_salary).toLocaleString()}</td>
                  </tr>
                )) : (
                  <tr><td colSpan="5" className="text-center py-2">No Records</td></tr>
                )}
              </tbody>
            </table>
            ...
            {payrollData.length > itemsPerPage && (
              <div className="flex justify-between items-center mt-4 pagination">
                ...
                <button disabled={currentPage === totalPagesPayroll} onClick={() => setCurrentPage(p => p + 1)} className="px-3 py-1 bg-gray-200 rounded disabled:opacity-50">Next</button>
              </div>
            )}
